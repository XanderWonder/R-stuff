...
SUSY_Training <- SUSY_Normalised[1:170,]
sUSY_Test <- SUSY_Normalised[170:340,]

SUSYK_Value <- floor(sqrt(length(SUSY_Training[,1])))

SUSY_Predictions <- knn(SUSY_Training,sUSY_Test,susydata[1:170,1], k=SUSYK_Value)

SUSY_Reference <- SUSY_NoID[170:340,1]
...
for(i in c(1:100)) {
  ...
  next_error <-  table(knn(SUSY_Training,sUSY_Test,susydata[1:170,1],k=i),SUSY_Reference)[1,2] +
    table(knn(SUSY_Training,sUSY_Test,susydata[1:170,1],k=i),SUSY_Reference)[2,1]
